...
final class VoiceNoteEntityTests: XCTestCase {
    ...
    func test_VoiceNote생성후_조회시_모든속성과관계가복원된다() throws {
        // Given
    ...
    func test_VoiceNote수정후_다시조회시_제목변경이반영된다() throws {
        // Given
        let store = try makeStore()
    ...
    func test_Transcript가없는상태에서_Transcript추가시_정상적으로반영된다() throws {
    ...
    func test_전사본만있는상태에서_요약과키워드추가시_모두정상반영된다() throws {
    ...
    func test_키워드목록이변경될때_업데이트시_삭제와추가가모두반영된다() throws {
        // Given — 키워드 A, B로 생성
        let store = try makeStore()
        let folder = Folder(name: "폴더")
        _ = try store.create(folder, as: FolderEntity.self)
    ...
    func test_동일데이터로_업데이트시_변경없이정상동작한다() throws {
        ...
        _ = try store.create(voiceNote, as: VoiceNoteEntity.self)
        let fetchedNote = try store.fetch(byID: voiceNote.id, as: VoiceNoteEntity.self)

        // When — 동일한 데이터로 update (toDomain() == domain이므로 조기 반환)
        _ = try store.update(fetchedNote, as: VoiceNoteEntity.self)
    ...
    func test_폴더존재상태에서_VoiceNote생성시_정상저장된다() throws {
        ...
        // When
        let saved = try store.create(voiceNote, as: VoiceNoteEntity.self)

        // Then
        let fetched = try store.fetch(byID: saved.id, as: VoiceNoteEntity.self)
        XCTAssertEqual(fetched.id, voiceNote.id)
    ...
    func test_여러노트가존재할때_전체조회시_수정일내림차순으로정렬된다() throws {
        // Given
        let store = try makeStore()
        ...
        // 의도적으로 순서를 뒤섞어 생성
        _ = try store.create(noteNewest, as: VoiceNoteEntity.self)
        _ = try store.create(noteOldest, as: VoiceNoteEntity.self)
        _ = try store.create(noteMiddle, as: VoiceNoteEntity.self)

        // When
        let allNotes = try store.fetchAll(VoiceNoteEntity.self)
    ...
    func test_DB에저장후_조회시_원본도메인객체와동일하다() throws {
        ...
        // When
        _ = try store.create(voiceNote, as: VoiceNoteEntity.self)
        let restored = try store.fetch(byID: voiceNote.id, as: VoiceNoteEntity.self)
    ...
    func test_VoiceNote삭제후_다시조회시_fetchFailed에러를던진다() throws {
        // Given
        let store = try makeStore()
        ...
        // Then
        do {
            _ = try store.fetch(byID: voiceNote.id, as: VoiceNoteEntity.self)
    ...
    func test_선택적관계가nil인노트를_생성후조회시_nil로정상복원된다() throws {
        // Given
        let store = try makeStore()
    ...
    func test_Transcript가있는노트에서_이를nil로변경후업데이트시_삭제가정상반영된다() throws {
