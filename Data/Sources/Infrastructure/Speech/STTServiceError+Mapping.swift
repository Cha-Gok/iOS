...
extension STTServiceError {
    ...
    private enum SpeechErrorCode {
        ...
        static let speechRecognizerDomain = "com.apple.speech.speechrecognitionframework"
        static let afAssistantDomain = "kAFAssistantErrorDomain"
    }

    init(_ error: Error) {
        let nsError = error as NSError
        if nsError.domain == SpeechErrorCode.speechRecognizerDomain,
           nsError.code == SpeechErrorCode.cancelled
        {
            self = .cancelled
