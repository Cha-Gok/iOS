...
public protocol AvailableModelSupportRepository: Sendable {
    ...
    func downloadModel(progressHandler: @Sendable @escaping (Progress) -> Void) async throws(AvailableModelSupportRepositoryError)
    
    /// 모델이 현재 메모리에 로드되어 즉시 사용 가능한 상태인지 확인합니다.
